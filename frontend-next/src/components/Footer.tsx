...
import {
  FileText, LayoutTemplate, Lightbulb, Image, Camera, Smartphone, Monitor,
  Users, Mail, Heart, Shield, ScrollText, BookOpen, CheckCircle
} from 'lucide-react';
...
export default function Footer() {
  ...
  const handleSubscribe = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.success(res.data.message || 'Thanks for subscribing! Check your email 🎉', { duration: 5000, icon: '💜' });
      setEmail('');
    } catch (err: unknown) {
        ...
        <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-2xl p-6 md:p-8 mb-10 md:mb-12 border border-white/10 backdrop-blur-sm">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              ...
              />
              <motion.button type="submit" disabled={loading} whileHover={!loading ? { scale: 1.05 } : {}} whileTap={!loading ? { scale: 0.95 } : {}} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-8 md:gap-10">
          ...
          <div className="col-span-2 lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-5 group">
              <motion.div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30" whileHover={{ rotate: 5, scale: 1.05 }}>
                <FileText className="w-7 h-7 text-white" />
              </motion.div>
              <span className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Ezy CV</span>
            </Link>
            ...
            <div className="flex gap-3">
              {socialLinks.map((s) => (
                <motion.a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" whileHover={{ y: -3 }} whileTap={{ scale: 0.95 }} className={`w-11 h-11 bg-white/10 rounded-xl flex items-center justify-center ${s.hoverBg} transition-all duration-300 border border-white/10`} title={s.name}>
                  {s.icon}
                </motion.a>
              ))}
            </div>
          ...
          {Object.entries(footerLinks).map(([title, links], idx) => (
            <motion.div key={title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} viewport={{ once: true }}>
              <h3 className="text-white font-bold mb-5 text-lg">{title}</h3>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link href={link.href} className="flex items-center gap-2 text-gray-400 hover:text-white transition-all duration-300 text-sm group">
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-6">
            <Link href="/privacy" className="text-gray-400 hover:text-white text-sm transition-all duration-300 hover:underline underline-offset-4">Privacy</Link>
            <Link href="/terms" className="text-gray-400 hover:text-white text-sm transition-all duration-300 hover:underline underline-offset-4">Terms</Link>
            <Link href="/cookies" className="text-gray-400 hover:text-white text-sm transition-all duration-300 hover:underline underline-offset-4">Cookies</Link>
            <Link href="/support-us" className="text-pink-400 hover:text-pink-300 text-sm font-medium transition-all duration-300 hover:underline underline-offset-4">❤️ Support Us</Link>
          </div>
        </div>
      </div>
    </footer>
